...
class ApplicationController < ActionController::Base
  ...
  before_action :remove_api_key_if_expired
  ...
  def remove_api_key_if_expired
    return if request.path.include?("forum/auth/")
    

    if current_user && !current_user.admin?
      key = Discourse.redis.get("API-KEY:#{current_user.id}")
      if key.nil?
        ApiKey.where(user_id: current_user.id).destroy_all
        raise StandardError.new("Your session has expired.")
      end
    end
  end

  def handle_session_expired(exception)
