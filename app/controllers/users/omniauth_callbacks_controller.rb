...
class Users::OmniauthCallbacksController < ApplicationController
  ...
  def complete
    ...
    if session.delete(:auth_reconnect) && authenticator.can_connect_existing_user? && current_user
      # Rails.logger.info(">>>>>>>>>>>>>>>>>>>> 1 if")
      path = persist_auth_token(auth)
      return redirect_to path
    else
      # Rails.logger.info(">>>>>>>>>>>>>>>>>>>> 1 else")
      Rails.logger.info(">>>>>>>>>>>>>>>>>>>> auth: #{auth}");
      ...
      # Rails.logger.info(">>>>>>>>>>>>>>>>>>>> @auth_result.user: #{@auth_result.user}");
    ...
    _user = @auth_result.user || current_user
    if _user
      ApiKey.where(user_id: _user.id).destroy_all
      key = ApiKey.create!(created_by_id: Discourse.system_user, user_id: _user.id, description: "API key for user #{_user.username}").key
      Rails.logger.info(">>>>>>>>key #{key}")
      Discourse.redis.setex("API-KEY:#{_user.id}", 5.minutes.to_i, key)
    end
    ...
    if _user
      ApiKey.where(user_id: _user.id).destroy_all
      key = ApiKey.create!(created_by_id: Discourse.system_user, user_id: _user.id, description: "API key for user #{_user.username}").key
      ttl = ENV["KEY_EXPIRY_TIME"].to_i
      ttl = 24.hours.to_i if ttl.zero?
      Discourse.redis.setex("API-KEY:#{_user.id}", ttl, key)
    end
